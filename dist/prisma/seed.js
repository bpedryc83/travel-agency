...
function getTripPhotos() {
    return [
        {
            id: '9aab0e96-886c-417a-b8ba-2d246ea662c4',
        ...
        {
            id: 'ce4b3293-e742-479e-9939-bb9630c94282',
        ...
        {
            id: '97c1417a-9d84-49e1-92bd-905679851011',
        ...
        {
            id: '4556dbcf-63ae-4f47-9328-8c3ab2ee43d7',
        ...
        {
            id: '7d8c684a-1737-4ab2-bf09-6af310810474',
...
async function seed() {
    await Promise.all(getTrips().map((trip) => {
        return db.trip.create({ data: trip });
    }));
    await Promise.all(getTripPhotos().map(({ tripId, ...tripPhotoData }) => {
    ...
    await Promise.all(getPasswords().map((password) => {
        return db.password.create({ data: password });
    }));
    await Promise.all(getOrders().map(({ userId, trips, ...orderData }) => {
